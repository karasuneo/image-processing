...
void DFT(cv::Mat &complex_mat, cv::Mat complexFT_mat){
    cv::Vec2d s; //色値
    cv::Mat tmp = complex_mat;
    for(int y=0; y<complex_mat.rows; y++){
      for(int x=0; x<complex_mat.cols; x++){
          complexFT_mat.at<cv::Vec2d>(y, x) = complex_mat.at<cv::Vec2d>(, x);
      }
    }
    

      //dft_idft(complexFT_mat, complex_mat.rows);
//
//      for(int x = 0; x<complex_mat.cols; x++){
////          complexFT_mat.at<cv::Vec2d>(y*complex_mat.cols+x, x*complex_mat.cols+y) = complex_mat.at<cv::Vec2d>(y, x);
//          complexFT_mat.at<cv::Vec2d>(y, x) = complex_mat.at<cv::Vec2d>(complex_mat.rows-x, complex_mat.cols-y);
//
//      }
//    }
}
...
int main(int argc, const char * argv[]) {
    ...
    //領域確保
    //convertTo:指定された型に変換する
    src_img.convertTo(real_mat, CV_64FC1);//実部
    //cv::Mat::zero: 全要素が 0 の行ベクトルを生成
    ...
    //離散フーリエ変換
    //左にフーリエ変換したい変数(?)を、右に出力用の変数を入れる
    //cv::dft(complex_mat, complexFT_mat);
    DFT(complex_mat, complexFT_mat);
